...
//			foreach ( $childpost as $children) {
				...
				if ($attachments) {
					...
					// echo apply_filters( 'the_title' , $attachment->post_title );
					$imageloader =  wp_get_attachment_image_src($attachment->ID,'thumbnail');
					$Captions = $attachment->post_excerpt;
					if ($Captions == 'film') {$slidecounter = $slidecounter - 1 ; $gallery_url = get_attachment_link( $attachment->ID );} else {$gallery_url =  get_permalink($childpost[1]['postID']) . "?slide_id=" .  $slidecounter;}
